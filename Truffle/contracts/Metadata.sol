...
contract Metadata {
	...
	string public name = "Bulls & Bears Game";
	...
	constructor(string memory _bullBase, string memory _fishermenBase, string[] memory _colors) {
		info.owner = msg.sender;
		info.bullTraits.base = _bullBase;
		info.bearTraits.base = _fishermenBase;
		info.colors = _colors;
	}
	...
	function createTrait(bool _isWhale, string memory _trait, string[] memory _names, string[] memory _imgs) external _onlyOwner {
		...
		Traits storage _traits = _isWhale ? info.bullTraits : info.bearTraits;
	...
	function setGame(GameInterface _wg) external _onlyOwner {
		info.wg = _wg;
	}

	function deploySetGame(GameInterface _wg) external {
		require(tx.origin == owner() && GameAddress() == address(0x0));
		info.wg = _wg;
	}
